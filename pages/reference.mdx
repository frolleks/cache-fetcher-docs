...
```ts
head(url: string, options?: Options): Promise<{headers: any, status: number, isError: boolean, error: unknown}>
```
...
```ts
options(url: string, options?: Options): Promise<{options: any, isError: boolean, error: unknown}>
```
...
```ts
patch(url: string, body: any, contentType?: string, options?: Options): Promise<{data: any, isError: boolean, error: unknown}>
```
...
```jsx
const { data, isSubmitting, isError, error, patch } = usePatchFetcher();
...
```jsx
const { headers, status, isLoading, isError, error } =
  useHeadFetcher("/api/headers");
...
```jsx
const { options, isLoading, isError, error } =
  useOptionsFetcher("/api/options");
